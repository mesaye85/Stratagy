"""Utilities for cleaning and merging country statistics.

This module reads ``main.csv`` to clean column names, map countries to a
consistent naming scheme and plot Gini coefficients. The processed
results are saved in ``main_merged.csv``.
"""
...
def plot_gini(gini_df: pd.DataFrame) -> None:
    """Plot and save Gini coefficients for each country."""
...
def main() -> None:
    """Run the preprocessing and merging workflow."""
    ...
    # Plot the Gini coefficients for visualization
    plot_gini(gini_data)
    ...
    # Map inconsistent country names to a uniform representation
